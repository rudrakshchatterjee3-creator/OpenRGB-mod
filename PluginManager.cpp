...
#ifdef _WIN32
    /*---------------------------------------------------------*\
    | Auto-download Effects Plugin if missing                   |
    \*---------------------------------------------------------*/
    filesystem::path effects_plugin_path = plugins_dir / "OpenRGBEffectsPlugin.dll";
    if(!filesystem::exists(effects_plugin_path))
    {
        LOG_INFO("[PluginManager] Effects plugin not found. Downloading...");
        std::string cmd = "curl -sL https://codeberg.org/OpenRGB/OpenRGBEffectsPlugin/releases/download/release_candidate_1.0rc2/OpenRGBEffectsPlugin_1.0rc2_Windows_64_415dc20.dll -o \"" + effects_plugin_path.string() + "\"";
        system(cmd.c_str());
    }
#endif
